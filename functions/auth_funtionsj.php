...
function registerUser($username, $email, $password) {
    ...
    try {
        $stmt = $db->prepare("INSERT INTO jdp (user, email, password_hash) VALUES (?, ?, ?)");
...
function authenticateUser($username, $password) {
    ...
    $stmt = $db->prepare("SELECT id, user, password_hash, activo FROM jdp WHERE user = ? OR email = ? LIMIT 1");
    ...
    if (!$user['activo']) {
        throw new Exception("Esta cuenta no esta Activa");
    }
    ...
    if (password_needs_rehash($user['password_hash'], PASSWORD_BCRYPT, ['cost' => 12])) {
        ...
        $stmt = $db->prepare("UPDATE jdp SET password_hash = ? WHERE id = ?");
        $stmt->execute([$newHash, $user['id']]);
    }
    
    // Actualizar último acceso
    $stmt = $db->prepare("UPDATE jdp SET ultimo_acceso = NOW() WHERE id = ?");
